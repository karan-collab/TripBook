...
exports.resizeUserPhoto = catchAsync(async (req, res, next) => {
  ...
  req.file.filename =
    req.file.fieldname + '-' + req.user.id + '-' + Date.now() + '.jpeg';
...
exports.updateMe = catchAsync(async (req, res, next) => {
  //1) Error if user tries to update password(POST's password data)
  ...
  //3) Filtered the doc so only fields which are allowed to change can be changed
